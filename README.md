![docker image](https://github.com/timval-differ/nomad-demo/actions/workflows/docker-publish.yml/badge.svg)


# timval-differ's NOMAD Oasis Distribution

This is the NOMAD Oasis distribution of timval-differ.
...
2. Clone the repository or download the repository as a zip file.
    ...
    ```sh
    git clone https://github.com/timval-differ/nomad-demo.git
    cd nomad-demo
    ```
    ...
    ```sh
    curl-L -o nomad-demo.zip "https://github.com/timval-differ/nomad-demo/archive/main.zip"
    unzip nomad-demo.zip
    cd nomad-demo
    ```
...
> [!NOTE]
> When using Podman instead of Docker: "Use all Docker tools, including CLI, with the Podman engine without any reconfiguration. These tools connect to the default Podman socket, thereby enabling you to use all Docker commands with Podman. For example, you can run the `docker run` command on the Podman engine to start a container." [https://podman-desktop.io/docs/migrating-from-docker/managing-docker-compatibility](source)
...
your `docker-compose.yaml` with `ghcr.io/timval-differ/nomad-demo:main` for the services
...
and advanced configuration, see the [nomad-helm-charts](https://github.com/timval-differ/nomad-helm-charts) repository.
...
By default, no plugins are included in this distribution. You can find a list of available NOMAD plugins [here](https://nomad-lab.eu/prod/v1/oasis/gui/search/plugins). For a list of official plugins provided by FAIRmat, please see [here](https://github.com/timval-differ/.github/blob/main/profile/README.md). For inspiration, you can also check the list of [plugins that are installed on the production NOMAD deployment hosted by FAIRmat](https://gitlab.mpcdf.mpg.de/nomad-lab/nomad-distro/-/raw/main/pyproject.toml?ref_type=heads).
...
```sh
docker pull ghcr.io/timval-differ/nomad-demo/jupyter:main
```
...
By default, documentation is built using the [nomad-docs](https://github.com/timval-differ/nomad-docs) repository. However, if you'd like to customize the documentation for your Oasis instance, you can easily do so.

1. First, [fork the nomad-docs repository](https://github.com/timval-differ/nomad-docs/fork).
...
_I get an_ `Error response from daemon: Head "https://ghcr.io/v2/timval-differ/nomad-demo/manifests/main": unauthorized`
